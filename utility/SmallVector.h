#ifndef __SMALLVECTOR_H__
#define __SMALLVECTOR_H__
...
protected:
  ...
  */
  ...
  /* Space after 'm_first' is clobbered, do not add any instance vars after it. */

protected:
	SmallVectorBase(size_t Size)
		:m_begin(&m_first), 
		m_end(&m_first),
		m_capacity((char*)&m_first+Size) 
	{
		return;
	}
	/*
	...
	*/
	bool isSmall() const 
	{
		return m_begin == static_cast<const void*>(&m_first);
	}
...
public:
	...
	{
		return size_t((char*)m_end - (char*)m_begin);
	}

	/* capacity_in_bytes - This returns capacity()*sizeof(T). */
	size_t capacity_in_bytes() const 
	{
		return size_t((char*)m_capacity - (char*)m_begin);
	}
...
void SmallVectorTemplateBase<T, isPodLike>::grow(size_t MinSize) 
{
	...
	this->m_capacity = this->begin()+NewCapacity;
}
...
namespace std 
{
	...
	template<typename T>
	inline void
		swap(SmallVectorImpl<T> &left, SmallVectorImpl<T> &right) 
	{
		left.swap(right);
	}
	...
	template<typename T, unsigned N>
	inline void
		swap(SmallVector<T, N> &left, SmallVector<T, N> &right) 
	{
		left.swap(right);
	}
}
